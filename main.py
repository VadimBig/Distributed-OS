    ...
    def create_nodes(self, scenario_nodes):
        ...
        for node_id in scenario_nodes:
            ...
            if way_eq == "static":
                ...
                x_s, y_s = scenario_nodes[node_id]['x_start'], scenario_nodes[node_id]['y_start']
                x_e, y_e = scenario_nodes[node_id]['x_end'], scenario_nodes[node_id]['y_end']
                way_equation = lambda x0, y0, d, t: constraints_to_brownian(brownian(x0, y0, n, t, w, out=None), x_s,
                                                                            y_s, x_e, y_e)
            else:
...
def generate_tasks(list_node_ids: list[str]) -> list[tuple]:
    ...
    * `time_to_create` - время появления задачи в симуляции
    ...
    for i in list_node_ids:
        count_tasks_node_i = int(
            np.random.exponential(scale=2.0, size=1))  # количество задач на узел по экспоненциальному распределению
        for j in range(count_tasks_node_i):
            calc_size = int(np.abs(
                np.random.normal(loc=150.0, scale=100, size=1)))  # распределение вычислительной сложности задачи?
            transfer_weight = int(
                np.abs(np.random.normal(loc=150.0, scale=100, size=1)))  # распределение размера задачи?
            transfer_weight_return = int(
                np.abs(np.random.normal(loc=150.0, scale=100, size=1)))  # распределение размера ответа
            time_to_create = prev_time + float(np.random.exponential(scale=100.0,
                                                                     size=1))  # задаём время, когда должна появится задача по экспоненциальному распределению
...
for node_id in scenario['nodes']:
    ...
    if way_eq == "static":
        ...
        x_s, y_s = scenario['nodes'][node_id]['x_start'], scenario['nodes'][node_id]['y_start']
        x_e, y_e = scenario['nodes'][node_id]['x_end'], scenario['nodes'][node_id]['y_end']
        way_equation = lambda x0, y0, d, t: constraints_to_brownian(brownian(x0, y0, n, t, w, out=None), x_s, y_s, x_e,
                                                                    y_e)
    else:
        print(f"Для узла {node_id} задано не реализованное уравнение движения - {way_eq}.")
        raise ValueError
    ...
    # задаём узел
    nodes[node_id] = Node(scenario['nodes'][node_id]['x'], scenario['nodes'][node_id]['y'],
                          scenario['nodes'][node_id]['power'], way_equation, direction)
